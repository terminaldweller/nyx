...
import memory_profiler.profile
...
def log_file_path(controller):
    ...
    for log_entry in controller.get_conf("Log", [], True):
        entry_comp = (
            log_entry.encode("ascii").decode("unicode_escape").split()
        )  # compensating for tab delimitation, looking for an entry like: notice file /var/log/tor/notices.log
...
class LogEntry(object):
    ...
    **Note:** Tor doesn't include the date in its timestamps so the year
    ...
    def __init__(self, timestamp, type, message):
        ...
        self.display_message = "%02i:%02i:%02i [%s] %s" % (
            entry_time[3],
            entry_time[4],
            entry_time[5],
            self.type,
        ...
        if GROUP_BY_DAY:
            self.dedup_key = "%s:%s:%s" % (
                self.type,
                self.day_count(),
                self._message_dedup_key(),
            )
        else:
    ...
    def clone(self):
        ...
        copy.duplicates = (
            None if self.duplicates is None else list(self.duplicates)
        )
        ...
        if path:
            try:
                ...
                stem.util.log.notice(
                    "nyx %s opening log file (%s)" % (nyx.__version__, path)
                )
            except (IOError, OSError) as exc:
                stem.util.log.error(
                    "Unable to write to log file: %s" % exc.strerror
                )
    ...
    def select(self, regex):
        with self._lock:
            ...
            try:
                ...
                stem.util.log.notice(
                    "Invalid regular expression pattern (%s): %s"
                    % (exc, regex)
                )
...
def read_tor_log(path, read_limit=None):
    ...
    for line in stem.util.system.tail(path, read_limit):
        ...
        if len(line_comp) < 4:
            raise ValueError(
                "Log located at %s has a line that doesn't match the format we expect: %s"
                % (path, line)
            )
        elif (
            len(line_comp[3]) < 3
            or line_comp[3][1:-1].upper() not in TOR_RUNLEVELS
        ):
            raise ValueError(
                "Log located at %s has an unrecognized runlevel: %s"
                % (path, line_comp[3])
            )
        ...
        try:
            ...
            timestamp_comp = list(
                time.strptime(timestamp_str, "%Y %b %d %H:%M:%S")
            )
            timestamp_comp[8] = isdst

            timestamp = int(
                time.mktime(tuple(timestamp_comp))
            ...
            raise ValueError(
                "Log located at %s has a timestamp we don't recognize: %s"
                % (path, " ".join(line_comp[:3]))
            )
    ...
    stem.util.log.info(
        "Read %s entries from tor's log file: %s (read limit: %s, runtime: %0.3f)"
        % (
            count,
            path,
            read_limit if read_limit else "none",
